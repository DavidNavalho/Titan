                ...
                for (;;) {
                    try {
                        ...
                        for( ClientPubInfo i : lstSession ) 
                        	nextTime = Math.min(nextTime, i.dumpNotificationsIfTimeout(clk));
    ...
    public void notify(CRDTIdentifier id, DHTSendNotification notification) {
        ...
        synchronized (cltsObserving) {
            ...
            for( ClientPubInfo i : map.values() ) 
            	i.addNotifications( notification.getInfo(), getEstimatedDCVersionCopy() );
                    ...
                    for( int i = 0; i < results.length; i++) {
                        ...
                        if( result.hasNotification()) {
                            if( results[i].isNotificationOnly()) {
                            	Thread.dumpStack();
                                PubSub.publish( result.getId(), new DHTSendNotification(result.getInfo().cloneNotification(), estimatedDCVersionCopy));
                            } else {
                                PubSub.publish(result.getId(), new DHTSendNotification(result.getInfo(), estimatedDCVersionCopy));
                            }
