...
public class RegisterVersioned<V extends Copyable> extends BaseCRDT<RegisterVersioned<V>> {
    ...
    protected SortedSet<QueueEntry<V>> values;
    ...
    // Copy constructor
    protected RegisterVersioned(RegisterVersioned other) {
        this.values = new TreeSet<QueueEntry<V>>();
        for (QueueEntry<V> e : values) {
            other.values.add(e.copy());
        }
        copyBase(this);
    }
    ...
    protected V value(CausalityClock versionClock) {
        for (QueueEntry<V> e : values) {
    ...
    public RegisterVersioned<V> copy() {
        return new RegisterVersioned<V>(this);
    }
